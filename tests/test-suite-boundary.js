/**
 * Boundary Test Suite Runner
 * Executes all boundary value test cases
 * 
 * @module test-suite-boundary
 * @description Runs all 10 boundary test files with 150 test cases
 */
...
class BoundaryTestSuiteRunner {
  ...
  /**
   * Run all boundary test files
   */
  async runAllTests() {
    const testFiles = [
      'boundary/test-max-quantity.js',
      'boundary/test-min-quantity.js',
      'boundary/test-max-cart-items.js',
      'boundary/test-zero-price.js',
      'boundary/test-max-price.js',
      'boundary/test-localstorage-quota.js',
      'boundary/test-long-product-name.js',
      'boundary/test-special-characters.js',
      'boundary/test-decimal-quantity.js',
      'boundary/test-cart-total-overflow.js'
    ];
  ...
  generateReport() {
    ...
    console.log('\n' + '='.repeat(80));
    console.log('BOUNDARY TEST SUITE RESULTS');
    console.log('='.repeat(80));
  ...
  async saveReport(format = 'json') {
    const reportDir = path.join(__dirname, 'reports', 'boundary');
    ...
    if (format === 'json') {
      const reportPath = path.join(reportDir, 'boundary-test-results.json');
      fs.writeFileSync(reportPath, JSON.stringify(this.results, null, 2));
      console.log(`📄 JSON report saved: ${reportPath}`);
    }

    if (format === 'html') {
      const reportPath = path.join(reportDir, 'boundary-test-report.html');
...
<head>
  ...
  <title>Boundary Test Suite Report</title>
  <style>
...
<body>
  <div class="container">
    <h1>🧪 Boundary Test Suite Report</h1>
...
if (require.main === module) {
  const runner = new BoundaryTestSuiteRunner();
  runner.run()
    .then(results => {
...
module.exports = BoundaryTestSuiteRunner;
